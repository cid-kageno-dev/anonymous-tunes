...
export const api = {
  ...
  genres: () => fetchAPI<{ data: any[] }>(`/genre`).then(r => r.data.filter(g => g.id !== 0)),
  genreArtists: (id: number) => fetchAPI<{ data: Artist[] }>(`/genre/${id}/artists`).then(r => r.data),
  editorialReleases: (id: number) => fetchAPI<{ data: Album[] }>(`/editorial/${id}/releases`).then(r => r.data),
  editorialSelection: (id: number) => fetchAPI<{ data: Album[] }>(`/editorial/${id}/selection`).then(r => r.data),
